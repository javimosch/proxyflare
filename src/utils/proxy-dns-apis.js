...
const LETENCRYPT_EMAILS = (process.env.letsEncryptContactEmails || "")
  .split(",")
  .filter((str) => !!str);
const NGINX_PROXY_MANAGER_HOST =
  process.env.nginxProxyManagerHost || "http://localhost:81";
const CLOUDFLARE_API_URL =
  process.env.CLOUDFLARE_API_URL || "https://api.cloudflare.com";
...
module.exports = scope = {
  async getToken(identity = NGINX_USERNAME, secret = NGINX_PASSWORD) {
    try {
      const response = await axios.post(
        `${NGINX_PROXY_MANAGER_HOST}/api/tokens`,
        {
          identity,
          secret,
        }
  ...
  async createOrUpdateProxyHost(
    ...
    try {
      ...
      const existingProxyHosts = await axios.get(
        `${NGINX_PROXY_MANAGER_HOST}/api/nginx/proxy-hosts`,
        {
      ...
      if (proxyHost) {
        ...
        await axios.put(
          `${NGINX_PROXY_MANAGER_HOST}/api/nginx/proxy-hosts/${proxyHostId}`,
          {
      ...
      const existingCertificates = await axios.get(
        `${NGINX_PROXY_MANAGER_HOST}/api/nginx/certificates`,
        {
          headers: { Authorization: `Bearer ${accessToken}` },
        }
      );
      ...
      if (!certificate) {
        certificate = existingCertificates.data.find((cert) =>
          cert.domain_names.includes(
            `*.${domains[0].split(".").slice(1).join(".")}`
          )
        );
      }
      ...
      console.error(
        "Error creating or updating proxy host with SSL:",
  ...
  async createOrUpdateDnsRecord(
    name,
    content = process.env.cloudflareDefaultARecordValue,
    zoneName = "",
    type = "A",
    proxied = false,
    ttl = 3600,
    cloudflareApiToken = process.env.cloudflareApiToken
  ) {
    // Parse the zoneNameToIdMap from the environment variable
    let zoneNameToIdMap = {};
    try {
      zoneNameToIdMap = JSON.parse(
        process.env.CLOUDFLARE_ZONE_NAME_TO_ID_MAP || "{}"
      );
    } catch (error) {
    ...
    try {
      ...
      const existingRecordsResponse = await axios.get(
        `${CLOUDFLARE_API_URL}/client/v4/zones/${zoneId}/dns_records`,
        {
          params: { type, name },
          headers: {
      ...
      if (existingRecords && existingRecords.length > 0) {
        ...
        const updateResponse = await axios.put(
          `${CLOUDFLARE_API_URL}/client/v4/zones/${zoneId}/dns_records/${existingRecord.id}`,
          {
            type,
            name,
            content,
      ...
      console.error(
        "Error creating or updating DNS record:",
        error.response ? error.response.data : error.message
      );
      throw error;
    }
  ...
  async getZones(cloudflareApiToken) {
    try {
      const response = await axios.get(
        "${CLOUDFLARE_API_URL}/client/v4/zones",
        {
          headers: {
            Authorization: `Bearer ${cloudflareApiToken}`,
          },
